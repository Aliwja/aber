...
type Category = { id: string; code: string; name_ar: string };
...
export default function HomePage() {
    ...
    async function loadData() {
      try {
        ...
        const [cats, cos, cts, biz] = await Promise.all([
          ...
          supabase
            .from('businesses')
            .select('id, display_name_ar, display_name_en, slug, cover_image_url, is_verified')
            .eq('status', 'APPROVED')
            .is('deleted_at', null)
  ...
  return (
    <div className="aber-layout">
      <Sidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />

      <div className="aber-main">
        {/* Topbar */}
        <div className="aber-topbar">
        ...
        {/* Content */}
        <div className="aber-content">
          {/* Search */}
          <div style={{ marginBottom: '16px' }}>
            <div className="search-bar">
              <span>🔍</span>
              <input
                placeholder="ابحث عن دولة، مدينة، فندق..."
                readOnly
              />
            </div>
          </div>
          ...
          {/* Categories */}
          {categories.length > 0 && (
            <div className="cat-row" style={{ padding: '12px 0' }}>
          ...
          {cities.length > 0 && (
            <section className="section" style={{ padding: '24px 0' }}>
              <div className="section-head">
                ...
                <a className="section-link" href="/countries">
                  عرض الكل ←
                </a>
              </div>
          ...
          <a
            href="/ai"
            className="ai-cta"
            style={{ textDecoration: 'none', margin: '24px 0' }}
          >
...
function getCategoryClass(code: string): string {
  const map: Record<string, string> = {
    hotels: 'hotel', resorts: 'hotel', apartments: 'hotel',
    restaurants: 'rest', cafes: 'rest',
    activities: 'act', adventure: 'act',
    experiences: 'exp', guides: 'exp', family: 'exp',
    tours: 'dest', transportation: 'dest',
    car_rental: 'dest', history: 'dest',
    events: 'event', shopping: 'event',
    beaches: 'hosp', nature: 'hosp',
  };
